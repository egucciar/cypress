...
describe "src/cy/commands/actions/click", ->
  ...
  beforeEach ->
    ...
    $(doc.body).empty().html('<div id="test"></div>')
    createShadowDom(doc.querySelector('#test'), @body);
  ...
  context "#click", ->
    ...
    it "bubbles up native click event", (done) ->
      ...
      cy.get('#test').shadowGet("#button").click()

    it "sends native mousedown event", (done) ->
      $btn = cy.$$shadow("#button", "#test")
    ...
    it "sends mousedown, mouseup, click events in order", ->
      ...
      cy.get('#test') .shadowGet("#button").click().then ->
    ...
    it "sends a click event", (done) ->
      cy.$$shadow("#button","#test").click -> done()

      cy.get('#test') .shadowGet("#button").click()

    it "returns the original subject", ->
      button = cy.$$shadow("#button","#test")

      cy.get('#test') .shadowGet("#button").click().then ($button) ->
        expect($button).to.match button

    # TODO: Findout why focus event did not occur?
    it.skip "causes focusable elements to receive focus", (done) ->
      $text = cy.$$(":text:first", "#test")

      $text.focus -> done()

      cy.get("#test")
        .shadowGet(":text:first")
        .click()

    it "does not fire a focus, mouseup, or click event when element has been removed on mousedown", ->
      $btn = cy.$$shadow("button:first", "#test")
      ...
      cy.get('#test')
        .shadowGet('div') # todo: implement .shadowRoot ? or .shadowContains ?
    ...
    # TODO: Not testing shadow dom here.
    ...
    it "causes first focused element to receive blur", ->
      ...
      cy.$$shadow("input:first", "#test").blur ->
        blurred = true

      cy.get("#test")
        .shadowGet("input:first").focus()
        .get("#test")
        .shadowGet("input:text:last").click()
        .then ->
    ...
    it "delays 50ms before resolving", ->
      cy.$$shadow("button:first", "#test").on "click", (e) =>
        cy.spy(Promise, "delay")

      cy.get("#test").shadowGet("button:first").click({ multiple: true}).then  ->
        expect(Promise.delay).to.be.calledWith(50, "click")

    it "can operate on a jquery collection", ->
      clicks = 0
      buttons = cy.$$shadow("button", "#test").slice(0, 3)
      buttons.click ->
        clicks += 1
        return false
      ...
      cy.get("#test").shadowGet("button").invoke("slice", 0, 3).click({ multiple: true}).then ($buttons) ->
    ...
    it "can cancel multiple clicks", (done) ->
      ...
      $anchors = cy.$$shadow("#sequential-clicks a", "#test")
      ...
      cy.get("#test")
        .shadowGet("#sequential-clicks a").click({ multiple: true })

    ## TODO: ShadowGet is not yielding the right thing here.
    it.skip "serially clicks a collection", ->
      clicks = 0
      ...
      cy.get("#test")
        .shadowGet("#sequential-clicks a").click({ multiple: true}).then ($anchors) ->
        expect($anchors.length).to.eq clicks

    it "increases the timeout delta after each click", ->
      count = cy.$$shadow("#three-buttons button", "#test").length

      cy.spy(cy, "timeout")

      cy.get("#test").shadowGet("#three-buttons button").click({ multiple: true}).then  ->
    ...
    it "can click elements which are huge and the center is naturally below the fold", ->
      cy.get("#test")
        .shadowGet("#massively-long-div").click()

    it "can click a tr", ->
      cy.get("#test")
        .shadowGet("#table tr:first").click()

    it "places cursor at the end of input", ->
      cy.get('#test').shadowGet('input:first').invoke('val', 'foobar').click().then ($el) ->
        el = $el.get(0)
        expect(el.selectionStart).to.eql(6)
        expect(el.selectionEnd).to.eql(6)
      cy.get('#test').shadowGet('input:first').invoke('val', '').click().then ($el) ->
        el = $el.get(0)
        expect(el.selectionStart).to.eql(0)
        expect(el.selectionEnd).to.eql(0)

    it "places cursor at the end of textarea", ->
      cy.get('#test').shadowGet('textarea:first').invoke('val', 'foo\nbar\nbaz').click().then ($el) ->
        el = $el.get(0)
        expect(el.selectionStart).to.eql(11)
        expect(el.selectionEnd).to.eql(11)

      cy.get('#test').shadowGet('textarea:first').invoke('val', '').click().then ($el) ->
        el = $el.get(0)
        expect(el.selectionStart).to.eql(0)
        expect(el.selectionEnd).to.eql(0)
    ...
    it "places cursor at the end of [contenteditable]", ->
      ...
      cy.get('#test').shadowGet('[contenteditable]:first')
    ...
    describe "actionability", ->
      it "can click elements which are hidden until scrolled within parent container", ->
        cy.get('#test').shadowGet("#overflow-auto-container").contains("quux").click()
      ...
      it "does not scroll when being forced", ->
        ...
        cy
          .get('#test').shadowGet("button:last").click({ force: true })
          .then ->
            expect(scrolled).to.be.empty

      ## TODO - Make Shadow
      it.skip "does not scroll when position sticky and display flex", ->
        scrolled = []
      ...
      it "can force click on hidden elements", ->
        cy.get("#test").shadowGet("button:first").invoke("hide").click({ force: true })

      it "can force click on disabled elements", ->
        cy.get('#test').shadowGet("input:first").invoke("prop", "disabled", true).click({ force: true })

      ## TODO: Covering Tests for shadow dom
      ...
      it.skip "scrolls the window past a fixed position element when being covered", ->
        $btn = $("<button>button covered</button>")
        ...
        cy.get('#test').shadowGet("#button-covered-in-nav").click().then ->
          ## - element scrollIntoView
          ## - element scrollIntoView (retry animation coords)
          ## - window
          expect(scrolled).to.deep.eq(["element", "element", "window"])

      it.skip "scrolls the window past two fixed positioned elements when being covered", ->
        $btn = $("<button>button covered</button>")
        .attr("id", "button-covered-in-nav")
        .appendTo(cy.$$("#fixed-nav-test"))
      ...
      it "waits until element becomes visible", ->
        $btn = cy.$$shadow("#button","#test").hide()
      ...
      it "waits until element is no longer disabled", ->
        $btn = cy.$$shadow("#button","#test").prop("disabled", true)
      ...
      it "waits until element stops animating", ->
        ...
        cy.get("#test").shadowGet("button:first").click().then ->
      ...
      it "does not throw when turning off waitForAnimations in options", ->
        ...
        cy.get("#test").shadowGet("button:first").click({waitForAnimations: false}).then ->
          expect(cy.ensureElementIsNotAnimating).not.to.be.called
      ...
      it "passes options.animationDistanceThreshold to cy.ensureElementIsNotAnimating", ->
        ...
        cy.get("#test").shadowGet("button:first").click({animationDistanceThreshold: 1000}).then ->
    ...
    describe "assertion verification", ->
      ...
      it "eventually passes the assertion", ->
        cy.$$shadow("button:first", "#test").click ->
          _.delay =>
            $(@).addClass("clicked")
          , 50
          return false

        cy.get("#test").shadowGet("button:first").click().should("have.class", "clicked").then ->
          lastLog = @lastLog
    ...
    describe.skip "errors", ->
      ...
      it "throws when attempting to click multiple elements", (done) ->
        num = cy.$$shadow("button", "#test").length
      ...
      it "throws when provided invalid position", (done) ->
        ...
        cy.get("#test").shadowGet("button:first").click("foo")
      ...
      it "throws when element animation exceeds timeout", (done) ->
        ...
        cy.$$shadow("button:first", "#test").on "click", ->
          clicks += 1
        ...
        cy.get("#test").shadowGet("button:first").click()
      ...
      it "eventually fails the assertion", (done) ->
        ...
        cy.get("#test").shadowGet("button:first").click().should("have.class", "clicked")
    ...
    describe.skip ".log", ->
      ...
      it "logs immediately before resolving", (done) ->
        button = cy.$$shadow("button:first", "#test")
      ...
      it "snapshots after clicking", ->
        cy.get("#test").shadowGet("button:first").click().then ($button) ->
          lastLog = @lastLog
  ...
  context "#dblclick", ->
    it "sends a dblclick event", (done) ->
      cy.$$shadow("#button","#test").dblclick (e) -> done()

      cy.get('#test') .shadowGet("#button").dblclick()

    it "returns the original subject", ->
      $btn = cy.$$shadow("#button","#test")

      cy.get('#test') .shadowGet("#button").dblclick().then ($button) ->
        expect($button).to.match $btn

    it "causes focusable elements to receive focus", (done) ->
      $text = cy.$$shadow(":text:first", "#test")

      $text.focus -> done()

      cy.get('#test').shadowGet(":text:first").dblclick()

    it "silences errors on unfocusable elements", ->
      cy.get('#test').shadowGet("div:first").dblclick()
    ...
    it "causes first focused element to receive blur", ->
      ...
      cy
        .get('#test').shadowGet("input:first").focus()
        .get('#test').shadowGet("input:text:last").dblclick()
        .then ->
          expect(blurred).to.be.true
    ...
    it "can operate on a jquery collection", ->
      ...
      $buttons = cy.$$shadow("button", "#test").slice(0, 2)
      $buttons.dblclick ->
        dblclicks += 1
        return false
      ...
      cy.get('#test').shadowGet("button").invoke("slice", 0, 2).dblclick().then ($buttons) ->
    ...
    it "serially dblclicks a collection", ->
      ...
      expect(anchors.length).to.be.gt 1
      cy.get('#test').shadowGet("#sequential-clicks a").dblclick().then ($anchors) ->
    ...
    it "increases the timeout delta after each dblclick", ->
      ...
      cy.get('#test').shadowGet("#three-buttons button").dblclick().then ->
        calls = cy.timeout.getCalls()
    ...
    describe "errors", ->
      ...
      it "throws when subject is not in the document", (done) ->
        ...
        $button = cy.$$shadow("button:first", "#test").dblclick (e) ->
          dblclicked += 1
          $button.remove()
          return false
        ...
        cy.get("#test").shadowGet("button:first").dblclick().dblclick()

      ## Failing test
      it.skip "throws when any member of the subject isnt visible", (done) ->
        $btn = cy.$$shadow("button", "#test").slice(0, 3).show().last().hide()
      ...
      ## TODO: failing test
      it.skip "throws when any member of the subject isnt visible", (done) ->
        $btn = cy.$$shadow("#three-buttons button", "#test").show().last().hide()
        ...
        cy.get('#test').shadowGet("#three-buttons button").dblclick()
    ...
    describe ".log", ->
      ...
      it "logs immediately before resolving", (done) ->
        ...
        cy.get("#test").shadowGet("button:first").dblclick()

      it "snapshots after clicking", ->
        cy.get("#test").shadowGet("button:first").dblclick().then ($button) ->
          lastLog = @lastLog
      ...
      it "logs only 1 dblclick event", ->
        ...
        cy.get("#test").shadowGet("button:first").dblclick().then ->
          expect(logs.length).to.eq(1)
